...
This UI is optional and sits on top of `matrix_operations.py`.
It does not require numpy except for the optional Random A feature.
"""
...
class MatrixCalculatorUI:
    ...
    def build_dimension_controls(self):
        ...
        tk.Button(frame, text="Create Matrices", command=self.create_matrices).grid(
    ...
    def build_buttons(self):
        ...
        tk.Button(frame, text="A + B", command=self.add).grid(row=0, column=0)

        tk.Button(frame, text="A - B", command=self.subtract).grid(row=0, column=1)

        tk.Button(frame, text="A × B", command=self.multiply).grid(row=0, column=2)

        tk.Button(frame, text="Scalar × A", command=self.scalar).grid(row=0, column=3)

        tk.Button(frame, text="Transpose A", command=self.transpose).grid(
            row=1, column=0
        )

        tk.Button(frame, text="Determinant A", command=self.det).grid(row=1, column=1)

        tk.Button(frame, text="Inverse A", command=self.inverse).grid(row=1, column=2)

        tk.Button(frame, text="Trace A", command=self.trace).grid(row=1, column=3)

        tk.Button(frame, text="Rank A", command=self.rank).grid(row=2, column=0)

        tk.Button(frame, text="Eigen A", command=self.eigen).grid(row=2, column=1)

        tk.Button(frame, text="Random A", command=self.random_A).grid(row=2, column=2)

        tk.Button(frame, text="Clear", command=self.clear).grid(row=2, column=3)
    ...
    def read_matrix(self, entries):
        ...
        for row in entries:
            ...
            for e in row:
                s = e.get().strip()
                if s == "":
                    raise ValueError("All matrix entries must be filled")
    ...
    def random_A(self):
        try:
            if np is None:
                raise ImportError("Random fill requires numpy")
            for row in self.entries_A:
                for e in row:
                    e.delete(0, tk.END)
...
def run_ui() -> None:
    """Entry point to run the Tkinter UI."""
    root = tk.Tk()
