...
class Matrix:
    ...
    def __init__(self, data: Sequence[Sequence[Number]]):
        ...
        # Preserve numeric types (int/float) but ensure list-of-lists
        self.data: List[List[Number]] = copy.deepcopy(data)  # type: ignore[assignment]
        self.rows: int = len(data)
        self.cols: int = len(data[0]) if len(data) > 0 else 0
    ...
    def _determinant_gaussian(self) -> float:
        ...
        n = self.rows
        # Work on a float copy to avoid mutating original and to keep numeric behavior stable
        matrix: List[List[float]] = [
            [float(x) for x in row] for row in copy.deepcopy(self.data)
        ]
        det = 1.0
    ...
    def trace(self) -> float:
        ...
        self.validate_not_empty()
        self.validate_square()
        return float(sum(self.data[i][i] for i in range(self.rows)))

    def rank(self, tol: float = 1e-12) -> int:
        """Compute matrix rank using Gaussian elimination."""
        self.validate_not_empty()

        m, n = self.rows, self.cols
        # Work on a float copy to keep numeric behavior consistent
        a = [[float(x) for x in row] for row in copy.deepcopy(self.data)]

        rank = 0
        row = 0
        for col in range(n):
            # Find a pivot row at/under the current row
            pivot = None
...
def matrix_eigen(A: Matrix) -> Tuple[Sequence[Any], Sequence[Sequence[Any]]]:
    """Return (eigenvalues, eigenvectors) using numpy if available.

    Eigenvector output matches numpy.linalg.eig: columns are eigenvectors.
    """
    A.validate_not_empty()
    A.validate_square()
    ...
    try:
        ...
        raise ImportError(
            "Eigen computation requires numpy. Install it with: pip install numpy"
        ) from e

    values, vectors = np.linalg.eig(np.array(A.data, dtype=complex))
